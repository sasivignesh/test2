...
/* ------------------------------------------------------------------ */
/*  Threading helpers (used only when MBEDTLS_THREADING_ALT is set)   */
/* ------------------------------------------------------------------ */
#if defined(MBEDTLS_THREADING_C) && defined(MBEDTLS_THREADING_ALT)

using mbedtls_threading_mutex_t = void *;

static void threading_mutex_init_cpp(mbedtls_threading_mutex_t *m) {
        if (!m) return;
        auto mt = new std::mutex;
        *m = static_cast<void *>(mt);
}
static void threading_mutex_free_cpp(mbedtls_threading_mutex_t *m) {
        if (!m) return;
        delete static_cast<std::mutex *>(*m);
        *m = nullptr;
}
static int threading_mutex_lock_cpp(mbedtls_threading_mutex_t *m) {
        if (!m || !*m) return MBEDTLS_ERR_THREADING_BAD_INPUT_DATA;
        static_cast<std::mutex *>(*m)->lock();
        return 0;
}
static int threading_mutex_unlock_cpp(mbedtls_threading_mutex_t *m) {
        if (!m || !*m) return MBEDTLS_ERR_THREADING_BAD_INPUT_DATA;
...
public:
        void randomize(uint8_t *buffer, size_t size) { sRNG->randomize(buffer, size); }

        std::unique_ptr<RNG> sRNG;
        mbedtlsStaticContexts() {
                mbedtls_threading_set_alt(threading_mutex_init_cpp,
                                          threading_mutex_free_cpp,
                                          threading_mutex_lock_cpp,
